...
export default function TemplateSelector({
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-2xl font-black text-white drop-shadow-lg">⚡ PILIH TEMPLATE RANGER</h3>
      <div className="grid grid-cols-3 gap-4">
        {templates.map((template, index) => {
          ...
          return (
            ...
            >
              <div className={`aspect-square bg-gradient-to-br ${colors[index % 5]} rounded-lg mb-2 flex items-center justify-center shadow-lg`}>
                <span className="text-5xl drop-shadow-lg">🦸</span>
              </div>
              <p className="text-sm font-black text-white drop-shadow-lg">{template.name}</p>
              {selectedTemplate?.id === template.id && (
