...
export default function CameraCapture({ onCapture, capturedImages }: CameraCaptureProps) {
  ...
  const capturePhoto = useCallback(() => {
    if (videoRef.current && canvasRef.current) {
      ...
      if (ctx) {
        ...
        const nextEmptyIndex = newImages.findIndex((img, idx) => img === null && idx > currentPhotoIndex);
      ...
      <div className="grid grid-cols-3 gap-2 mb-4">
        {[0, 1, 2].map((index) => (
          ...
          >
            ...
            {currentPhotoIndex === index && isStreaming && (
              <div className="absolute top-2 left-2 bg-accent-yellow text-black px-2 py-1 rounded text-xs font-bold animate-pulse">
                📸 Aktif
              </div>
            )}
          </div>
